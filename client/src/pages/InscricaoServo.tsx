...
import { ArrowLeft, ArrowRight, CheckCircle2, User, Heart, Phone, Church, Shield, ClipboardCheck, Loader2, AlertCircle } from "lucide-react";
...
const STEPS = [
  ...
  { id: 4, title: "Igreja", description: "Igreja e líder espiritual" },
      ...
      await createRegistration.mutateAsync({
        ...
        servant: {
          ...
          newLeader: leader.newLeader ? {
            title: leader.title,
            name: leader.name,
            phone: unmask(leader.phone),
            whatsapp: unmask(leader.whatsapp),
            email: leader.email,
          } : undefined,
        },
      });
    ...
    switch (currentStep) {
      ...
      case 3:
        return (church.denomination || church.churchName) &&
          (leader.spiritualLeaderId || (leader.newLeader && leader.name && leader.whatsapp && isValidPhone(leader.whatsapp)));
      case 4:
        ...
        >
            ...
            <Card className="bg-zinc-900 border-zinc-800 animate-in fade-in duration-300">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <ValidatedInput className="md:col-span-2" label="Nome Completo" required value={personal.fullName} onChange={(v) => setPersonal({ ...personal, fullName: v })} placeholder="Nome conforme documento" validate={(v) => v.length >= 3} errorMessage="Nome deve ter pelo menos 3 caracteres" />
                  <ValidatedInput label="CPF" required value={personal.cpf} onChange={(v) => setPersonal({ ...personal, cpf: v })} mask={maskCPF} validate={isValidCPF} placeholder="000.000.000-00" errorMessage="CPF inválido" />
                  <ValidatedInput label="RG" value={personal.rg} onChange={(v) => setPersonal({ ...personal, rg: v })} placeholder="0000000 SSP/RO" />
                  <div>
                  ...
                  <ValidatedInput label="Telefone Celular" required value={personal.phone} onChange={(v) => setPersonal({ ...personal, phone: v })} mask={maskPhone} validate={isValidPhone} placeholder="(69) 99999-9999" errorMessage="Telefone inválido (DDD + 9 dígitos)" />
                  <ValidatedInput label="WhatsApp" required value={personal.whatsapp} onChange={(v) => setPersonal({ ...personal, whatsapp: v })} mask={maskPhone} validate={isValidPhone} placeholder="(69) 99999-9999" errorMessage="WhatsApp inválido (DDD + 9 dígitos)" />
                  <ValidatedInput className="md:col-span-2" label="E-mail" required type="email" value={personal.email} onChange={(v) => setPersonal({ ...personal, email: v })} validate={isValidEmail} placeholder="seu@email.com" errorMessage="E-mail inválido" />
                  <ValidatedInput className="md:col-span-2" label="Endereço Completo" required value={personal.address} onChange={(v) => setPersonal({ ...personal, address: v })} placeholder="Rua, número, complemento" />
                  <ValidatedInput label="Bairro" required value={personal.neighborhood} onChange={(v) => setPersonal({ ...personal, neighborhood: v })} placeholder="Bairro" />
                  <ValidatedInput label="Cidade" required value={personal.city} onChange={(v) => setPersonal({ ...personal, city: v })} placeholder="Porto Velho" />
                  <ValidatedInput label="UF" required value={personal.state} onChange={(v) => setPersonal({ ...personal, state: v.toUpperCase() })} placeholder="RO" maxLength={2} validate={(v) => v.length === 2} errorMessage="UF deve ter 2 letras" />
                  <div className="relative">
                    <ValidatedInput label="CEP" required value={personal.zipCode} onChange={async (v) => {
                      setPersonal({ ...personal, zipCode: v });
                      const digits = v.replace(/\D/g, "");
                      if (digits.length === 8) {
                        const result = await cepLookup.lookup(v);
                        if (result) {
                          setPersonal(prev => ({ ...prev, zipCode: v, address: result.logradouro || prev.address, neighborhood: result.bairro || prev.neighborhood, city: result.localidade || prev.city, state: result.uf || prev.state }));
                          toast.success("Endereço preenchido automaticamente!");
                        }
                      }
                    }} mask={maskCEP} validate={isValidCEP} placeholder="76800-000" errorMessage="CEP inválido (8 dígitos)" />
                    {cepLookup.loading && <div className="absolute right-3 top-8"><Loader2 className="w-4 h-4 animate-spin text-primary" /></div>}
                    {cepLookup.error && <p className="text-xs text-amber-400 mt-1">{cepLookup.error}</p>}
                  </div>
                  <ValidatedInput label="Profissão" value={personal.profession} onChange={(v) => setPersonal({ ...personal, profession: v })} placeholder="Sua profissão" />
                  <div>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <Select value={medical.bloodType} onValueChange={(v) => setMedical({ ...medical, bloodType: v })}>
                    <SelectTrigger className="mt-1 bg-zinc-800 border-zinc-700"><SelectValue placeholder="Selecione" /></SelectTrigger>
                    <SelectContent>{BLOOD_TYPES.map(t => <SelectItem key={t} value={t}>{t === "nao_sei" ? "Não sei" : t}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-4">
                  ...
                  {medical.hasAllergy && <div className="animate-in fade-in slide-in-from-top-2 duration-200"><Textarea value={medical.allergyDetails} onChange={(e) => setMedical({ ...medical, allergyDetails: e.target.value })} placeholder="Descreva suas alergias (medicamentos, alimentos, etc.)" className="bg-zinc-800 border-zinc-700" /></div>}
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 bg-zinc-800 rounded-lg">
                  ...
                  {medical.hasMedication && <div className="animate-in fade-in slide-in-from-top-2 duration-200"><Textarea value={medical.medicationDetails} onChange={(e) => setMedical({ ...medical, medicationDetails: e.target.value })} placeholder="Liste os medicamentos que utiliza" className="bg-zinc-800 border-zinc-700" /></div>}
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 bg-zinc-800 rounded-lg">
                  ...
                  {medical.hasChronicDisease && <div className="animate-in fade-in slide-in-from-top-2 duration-200"><Textarea value={medical.chronicDiseaseDetails} onChange={(e) => setMedical({ ...medical, chronicDiseaseDetails: e.target.value })} placeholder="Descreva a doença e tratamento" className="bg-zinc-800 border-zinc-700" /></div>}
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 bg-zinc-800 rounded-lg">
                  ...
                  {medical.hasPhysicalRestriction && <div className="animate-in fade-in slide-in-from-top-2 duration-200"><Textarea value={medical.physicalRestrictionDetails} onChange={(e) => setMedical({ ...medical, physicalRestrictionDetails: e.target.value })} placeholder="Descreva a restrição (ex: não pode correr, problema no joelho)" className="bg-zinc-800 border-zinc-700" /></div>}
                </div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 bg-zinc-800 rounded-lg">
                  ...
                  {medical.hasFoodRestriction && <div className="animate-in fade-in slide-in-from-top-2 duration-200"><Textarea value={medical.foodRestrictionDetails} onChange={(e) => setMedical({ ...medical, foodRestrictionDetails: e.target.value })} placeholder="Descreva (ex: vegetariano, intolerância a lactose, celíaco)" className="bg-zinc-800 border-zinc-700" /></div>}
                </div>
                <ValidatedInput label="Plano de Saúde (nome e número do cartão)" value={medical.healthInsurance} onChange={(v) => setMedical({ ...medical, healthInsurance: v })} placeholder="Ex: Unimed 0000000000" />
                <div>
              ...
              <CardContent className="space-y-6">
                {emergencyContacts.map((contact, index) => (
                  <div key={index} className="p-4 border border-zinc-700 rounded-lg space-y-4 animate-in fade-in duration-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <ValidatedInput className="md:col-span-2" label="Nome Completo" required value={contact.name} onChange={(v) => updateEmergencyContact(index, "name", v)} placeholder="Nome do contato" />
                      <div>
                      ...
                      {contact.relationship === "outro" && (
                        <ValidatedInput label="Especifique o vínculo" value={contact.relationshipOther} onChange={(v) => updateEmergencyContact(index, "relationshipOther", v)} placeholder="Ex: padrinho, amigo" />
                      )}
                      <ValidatedInput label="Telefone" required value={contact.phone} onChange={(v) => updateEmergencyContact(index, "phone", v)} mask={maskPhone} validate={isValidPhone} placeholder="(69) 99999-9999" errorMessage="Telefone inválido" />
                      <ValidatedInput label="WhatsApp" required value={contact.whatsapp} onChange={(v) => updateEmergencyContact(index, "whatsapp", v)} mask={maskPhone} validate={isValidPhone} placeholder="(69) 99999-9999" errorMessage="WhatsApp inválido" />
                      <ValidatedInput label="E-mail" value={contact.email} onChange={(v) => updateEmergencyContact(index, "email", v)} validate={contact.email ? isValidEmail : undefined} placeholder="email@exemplo.com" errorMessage="E-mail inválido" />
                      <ValidatedInput label="Cidade" value={contact.city} onChange={(v) => updateEmergencyContact(index, "city", v)} placeholder="Cidade/UF" />
                    </div>
          ...
          {currentStep === 3 && (
            <Card className="bg-zinc-900 border-zinc-800 animate-in fade-in duration-300">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  {!church.newChurch ? (
                    ...
                    <div className="space-y-4 p-4 border border-zinc-700 rounded-lg mt-1 animate-in fade-in slide-in-from-top-2 duration-200">
                      ...
                      <ValidatedInput label="Nome da Igreja" required value={church.churchName} onChange={(v) => setChurch({ ...church, churchName: v })} placeholder="Nome da igreja" />
                      <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <ValidatedInput label="Membro desde" value={church.memberSince} onChange={(v) => setChurch({ ...church, memberSince: v })} placeholder="MM/AAAA" />
                  <ValidatedInput label="Função na igreja" value={church.ministryRole} onChange={(v) => setChurch({ ...church, ministryRole: v })} placeholder="Ex: líder de louvor, diácono" />
                </div>
                ...
                <div className="border-t border-zinc-700 pt-6">
                  ...
                  {!leader.newLeader ? (
                    ...
                    <div className="space-y-4 p-4 border border-zinc-700 rounded-lg animate-in fade-in slide-in-from-top-2 duration-200">
                      <p className="text-sm text-zinc-400">Cadastrar novo líder espiritual:</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label className="text-sm text-zinc-300">Título<span className="text-red-400 ml-0.5">*</span></Label>
                          <Select value={leader.title} onValueChange={(v) => setLeader({ ...leader, title: v })}>
                            <SelectTrigger className="mt-1 bg-zinc-800 border-zinc-700"><SelectValue placeholder="Título" /></SelectTrigger>
                            <SelectContent>{LEADER_TITLES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
                          </Select>
                        </div>
                        <ValidatedInput label="Nome Completo" required value={leader.name} onChange={(v) => setLeader({ ...leader, name: v })} placeholder="Nome do líder" />
                        <ValidatedInput label="Telefone" required value={leader.phone} onChange={(v) => setLeader({ ...leader, phone: v })} mask={maskPhone} validate={isValidPhone} placeholder="(69) 99999-9999" errorMessage="Telefone inválido" />
                        <ValidatedInput label="WhatsApp" required value={leader.whatsapp} onChange={(v) => setLeader({ ...leader, whatsapp: v })} mask={maskPhone} validate={isValidPhone} placeholder="(69) 99999-9999" errorMessage="WhatsApp inválido" />
                        <ValidatedInput className="md:col-span-2" label="E-mail" value={leader.email} onChange={(v) => setLeader({ ...leader, email: v })} validate={leader.email ? isValidEmail : undefined} placeholder="email@exemplo.com" errorMessage="E-mail inválido" />
                      </div>
          ...
          {currentStep === 4 && (
            <Card className="bg-zinc-900 border-zinc-800 animate-in fade-in duration-300">
              ...
              <CardContent className="space-y-6">
                <div>
                  <Label className="text-sm text-zinc-300">Função desejada no TOP<span className="text-red-400 ml-0.5">*</span></Label>
                  <Select value={servant.servantRole} onValueChange={(v) => setServant({ ...servant, servantRole: v })}>
                    <SelectTrigger className="mt-1 bg-zinc-800 border-zinc-700"><SelectValue placeholder="Selecione a área de serviço" /></SelectTrigger>
                    <SelectContent>{SERVANT_ROLES.map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
                  </Select>
                </div>

                <div>
                  <Label className="text-sm text-zinc-300">Quantos TOPs já serviu?</Label>
                  <Input type="number" min={0} value={servant.previousTops} onChange={(e) => setServant({ ...servant, previousTops: parseInt(e.target.value) || 0 })} className="mt-1 bg-zinc-800 border-zinc-700 text-white" />
                </div>

                <ValidatedInput label="Número de Legendário (se já participou como participante)" value={servant.legendaryNumber} onChange={(v) => setServant({ ...servant, legendaryNumber: v })} placeholder="Ex: 1670-042" />
                ...
                <div className="p-4 bg-zinc-800 rounded-lg">
                  ...
                  {emergencyContacts.map((c, i) => (
                    <div key={i} className="grid grid-cols-2 gap-2 text-sm mb-2">
                      <span className="text-zinc-400">Nome:</span><span className="text-white">{c.name} ({RELATIONSHIP_OPTIONS.find(o => o.value === c.relationship)?.label || c.relationship})</span>
