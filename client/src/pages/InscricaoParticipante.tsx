...
import { ArrowLeft, ArrowRight, CheckCircle2, User, Heart, Phone, Church, ClipboardCheck, Loader2 } from "lucide-react";
...
export default function InscricaoParticipante() {
  ...
  const canAdvance = useMemo(() => {
    switch (currentStep) {
      ...
      case 1:
        return personal.fullName && medical.bloodType;
      case 2:
        ...
        {currentStep === 1 && (
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                <Label>Plano de Saúde (nome e número do cartão)</Label>
              ...
              <div>
                <Label>Observações adicionais de saúde</Label>
                <Textarea value={medical.healthObservations} onChange={(e) => setMedical({ ...medical, healthObservations: e.target.value })} placeholder="Qualquer informação relevante para a equipe médica" className="bg-zinc-800 border-zinc-700" />
              </div>
            </CardContent>
          </Card>
        )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {contact.relationship === "outro" && (
                      <div>
                        ...
                        <Input value={contact.relationshipOther} onChange={(e) => updateEmergencyContact(index, "relationshipOther", e.target.value)} placeholder="Ex: padrinho, amigo" className="bg-zinc-800 border-zinc-700" />
                      </div>
                    )}
                    <div>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent className="space-y-6">
              ...
              {!church.newChurch && !church.churchId && (
                <div>
                  <Label>Denominação/Segmento *</Label>
                  <Select value={church.denomination} onValueChange={(v) => setChurch({ ...church, denomination: v })}>
                    <SelectTrigger className="bg-zinc-800 border-zinc-700"><SelectValue placeholder="Selecione" /></SelectTrigger>
                    <SelectContent>
                      {DENOMINATIONS.map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}
            ...
            <CardContent className="space-y-6">
              ...
              <div className="p-4 bg-zinc-800 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <span className="text-zinc-400">Estado Civil:</span><span className="text-white">{personal.maritalStatus}</span>
              ...
              <div className="p-4 bg-zinc-800 rounded-lg">
                ...
                {emergencyContacts.map((c, i) => (
                  <div key={i} className="grid grid-cols-2 gap-2 text-sm mb-2">
                    ...
                    <span className="text-zinc-400">Vínculo:</span><span className="text-white">{c.relationship}</span>
